...
На этапе 5: Реализовали команды rm и mkdir.

### ls

* -a — показывать скрытые файлы (с точкой).
* -l — длинный формат (тип + размер + имя).
* -h — человекочитаемые размеры (используется вместе с -l).
* -R — рекурсия (показ содержимого вложенных директорий).
...
Поддерживаются абсолютные и относительные пути внутри VFS.
...
Выдаёт ошибки для директорий или несуществующих файлов.
...
* -A — глобальный режим (учитывает повторения по всему файлу, а не только подряд).
* -f N — пропустить первые N строк перед сравнением (например, заголовки).
* -s N — при сравнении игнорировать первые N символов строки (печатается оригинальная строка).
...
13. реализованы rm и mkdir (работают внутри VFS)
...
пример работы команды mkdir:


![пример работы команды mkdir](image-13.png)

пример работы команды rm:


![пример работы команды rm](image-14.png)
